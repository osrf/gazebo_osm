...
class Osm2Dict:
  ...
  def __init__(self):
    ...
    self.addModel = dict({ "stop" : { "modelName" : "stop_sign", "occurence" : -1} , "street_lamp" : { "modelName" : "lamp_post", "occurence" : -1} , "traffic_signals" : {"modelName" : "construction_cone", "occurence" : -1}, "fire hydrant" : {"modelName": "fire_hydrant", "occurence" : -1}})#, "house": { "modelName" : "house_1", "occurence" : -1}, "post_office" : {'modelName': "office_building", 'occurence': -1}})
    ...
    while 1:
      ...
      if option == 'N':
         break;
      getMapImage( [min(startCoords[1], endCoords[1]), min(startCoords[0], endCoords[0]), max(startCoords[1], endCoords[1]), max(startCoords[0], endCoords[0])])
      final = raw_input("Do you want to continue (C) or select new coordinates (S)? (default:C)").upper()
      if final == 'S':
        continue
      break         
    ...
    for i in range( len(data) ):
      ...
      for modelNum in self.modelType:
        ...
        if tagData.get(modelNum) in self.addModel.keys() :
          ...
          self.models.update(dict({self.addModel[ modelType]['modelName'] + "_" + str(repNum): { "points" : modelLocation, 'mainModel': self.addModel[modelType]['modelName'] }})) 
